...
func main() {
	...
	for _, d := range []string{
		...
		if _, err := db.Exec(d); err != nil { fmt.Printf("FAIL %-70.70s %v\n", d, err) } else { fmt.Printf("OK   %-70.70s\n", d) }
	}
	for _, f := range []string{"vec_version()", "vec_f32('[1,2,3]')", "vec_quantize_int8(vec_f32('[0.1,0.2]'),'unit')", "vec_distance_cosine(vec_f32('[1,0]'),vec_f32('[0,1]'))", "vec_quantize_binary(vec_f32('[1,-1,1,-1,1,-1,1,-1]'))"} {
		var out any
		if err := db.QueryRow("select "+f).Scan(&out); err != nil { fmt.Printf("FAIL %-60s %v\n", f, err) } else { fmt.Printf("OK   %-60s\n", f) }
	}
}
