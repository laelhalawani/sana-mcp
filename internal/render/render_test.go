...
func TestParticipantWithoutEmailHasNoGap(t *testing.T) {
	out := Participants([]sana.Participant{
		{DisplayName: "Zara", Email: ptr("")},
		{DisplayName: "Dana", Email: ptr("dana@example.com"), IsHost: true},
	}, Styles{})
	if strings.Contains(out, "Zara  \n") || strings.Contains(out, "Zara   ") {
		t.Fatalf("an empty email must not print a gap, got %q", out)
	}
	if !strings.Contains(out, "Dana  dana@example.com  (host)") {
...
func TestSummaryRendersNotesAndActions(t *testing.T) {
	out := Summary(sana.Metadata{
		...
		ActionItems: []sana.ActionItem{
			{AssignedTo: ptr("Priya"), Action: "scale to 500 products", DueDate: ptr("2026-08-01")},
