...
func TestResplitRenumbersAndCarriesCorrectionsAcross(t *testing.T) {
	...
	if err := store.PutTranscript("m1", []Line{
		{LineNo: 1, Speaker: "Linda", StartMS: 0, OriginalText: "Yeah."},
		{LineNo: 2, Speaker: "Lael", StartMS: 5000, OriginalText: long},
		{LineNo: 3, Speaker: "Linda", StartMS: 9000, OriginalText: "Got it."},
	}); err != nil {
	...
	for _, index := range []int{1, 2, 3} {
		if lines[index].Speaker != "Lael" || lines[index].StartMS != 5000 {
...
func TestResplitIsIdempotent(t *testing.T) {
	...
	if err := store.PutTranscript("m1", []Line{
		{LineNo: 1, Speaker: "Lael", StartMS: 0,
