...
func TestALineCanBeCorrectedMoreThanOnce(t *testing.T) {
	...
	if err := database.PutTranscript("m1", []store.Line{
		{LineNo: 1, Speaker: "Dana", Text: original, OriginalText: original},
	}); err != nil {
		t.Fatal(err)
	}
...
func TestALateRecordingDoesNotLandOnAnotherDocument(t *testing.T) {
	...
	model.title, model.lines = "Transcript", []string{"1 [0:00] Dana: hello"}
	model.Update(stale)
...
func TestASavedCorrectionIsDrawnImmediately(t *testing.T) {
	...
	if err := database.PutTranscript("m1", []store.Line{
		{LineNo: 1, Speaker: "Dana", Text: "the Fabrics pipeline", OriginalText: "the Fabrics pipeline"},
	}); err != nil {
		t.Fatal(err)
	}
...
func TestScrollSurvivesAResize(t *testing.T) {
	...
	for index := range lines {
		lines[index] = fmt.Sprintf("%d [0:00] Dana: line %d", index+1, index+1)
	}
