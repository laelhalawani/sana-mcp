...
// Only serve the direct-entry case; under a compiled binary the CLI's `mcp`
// subcommand invokes runMcp() itself, so a guard on argv would double-fire.
if (!isCompiledBinary() && process.argv[1] === fileURLToPath(import.meta.url))
  runMcp().catch((e) => {
    console.error(e);
    process.exit(1);
  });
