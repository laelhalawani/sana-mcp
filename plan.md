...
built **by reading them** (`#11218` "do zestawienia między Fabrik, a Northwind, a
...
A ~10-line glossary (`fabrics -> fabrics|fabrix|fabriko|fabrik`) applied as query
expansion:
...
- **One genuine win:** distributional mining *does* find spelling near-misses for
  ...
  `fabrix`/`fabriko`/`fabriks`/`crawlery` with no LLM at all. It simply cannot
  bridge to `fabrik`.
...
1. Extract rare proper-noun candidates: **1,944 terms** at `3 <= df <= 2 %` of
   chunks (measured). `Fabrik`, `Fabrics`, `Fabrix`, `Fabriko`, `Vantik`, `Zenolith`
   are all in that set.
2. Harvest free wins locally first - edit distance + double metaphone (**both**
   codes) + distributional neighbours - clustering `fabrics`/`fabrix`/`fabriko`/
