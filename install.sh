...
# Pin a release:
#   SANA_MCP_VERSION=v0.4.4 sh -c 't=$(mktemp) && curl -fsSL "$1" -o "$t" && sh "$t"; s=$?; [ -z "${t:-}" ] || rm -f "$t"; exit "$s"' sh https://github.com/Lumen-AiApp/sana-ai-mcp/releases/download/v0.4.4/install.sh
set -eu
set -f
umask 077
