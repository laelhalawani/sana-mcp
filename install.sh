...
# Pin a release:
#   SANA_MCP_VERSION=v0.4.1 sh -c 't=$(mktemp) && curl -fsSL "$1" -o "$t" && sh "$t"; s=$?; [ -z "${t:-}" ] || rm -f "$t"; exit "$s"' sh https://github.com/Etals-AiApp/sana-ai-mcp/releases/download/v0.4.1/install.sh
set -eu
set -f
umask 077
