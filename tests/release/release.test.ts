...
test("release workflow binds every build and publish step to the authorized commit", async () => {
  ...
  assert.ok(
    (
      workflow.match(
        /GITHUB_REPOSITORY: \$\{\{ github\.repository \}\}/g,
      ) ?? []
    ).length >= 2,
  );
  assert.match(
  ...
  assert.ok(
    (
      workflow.match(
        /--commit-sha (?:["']?\$SOURCE_SHA["']?|\$env:SOURCE_SHA)|"--commit-sha", \$env:SOURCE_SHA/g,
      ) ?? []
    ).length >= 4,
  );
  assert.match(
  ...
  assert.match(workflow, /-v "\$EXECUTION_ASSET:\/execution\/artifact:ro"/);
  ...
  assert.match(
    workflow,
    /authorized_assets="\$publish_work\/authorized-assets"[\s\S]*cp -a release-assets\/\. "\$authorized_assets\/"[\s\S]*assembled-authority[\s\S]*exec \{asset_fd\}<"\$authorized_assets\/\$asset"[\s\S]*descriptor_digest=.*\/proc\/self\/fd\/\$asset_fd/,
  );
  assert.match(
    workflow,
    /missing_assets\+=\("\$asset"\)[\s\S]*reset_asset_descriptor "\$asset_fd"[\s\S]*upload-asset \\\n\s+--release-json "\$release_json" \\\n\s+--repository "\$GITHUB_REPOSITORY" \\\n\s+--asset-name "\$asset" \\\n\s+--fd "\$asset_fd" \\\n\s+--expected-sha256 "\$\{expected_digests\[\$asset\]\}" \\\n\s+--expected-origin "\$expected_upload_origin"/,
  );
  assert.match(
    workflow,
    /EXPECTED_UPLOAD_ORIGIN: \$\{\{ github\.server_url == 'https:\/\/github\.com' && 'https:\/\/uploads\.github\.com' \|\| '' \}\}[\s\S]*\[ "\$GITHUB_SERVER_URL" != "https:\/\/github\.com" \][\s\S]*\[ "\$GITHUB_API_URL" != "https:\/\/api\.github\.com" \][\s\S]*test -n "\$EXPECTED_UPLOAD_ORIGIN"[\s\S]*expected_upload_origin=\$EXPECTED_UPLOAD_ORIGIN/,
  );
  assert.match(workflow, /verify_remote_assets true/);
  assert.match(
    workflow,
    /expected_digests\["\$asset"\]=\$digest[\s\S]*test "\$\(sha256sum "\$download_dir\/\$asset"[\s\S]*= "\$\{expected_digests\[\$asset\]\}"/,
  );
  ...
  assert.match(
    workflow,
    /assembled-authority[\s\S]*> "\$authority_json"[\s\S]*test -s "\$authority_json"[\s\S]*authority\.files\.length !== expectedNames\.length[\s\S]*mapfile -t authority_lines < "\$authority_lines_file"/,
  );
  assert.doesNotMatch(
    workflow,
    /mapfile -t authority_lines < <\(/,
  );
  assert.match(
    workflow,
    /release\.id <= 0[\s\S]*typeof release\.draft !== "boolean"[\s\S]*release\.url !== expectedApiUrl/,
  );
  assert.match(
    workflow,
    /fetch_pinned_release\(\)[\s\S]*"repos\/\$GITHUB_REPOSITORY\/releases\/\$release_id"/,
  );
  assert.match(
    workflow,
  ...
  assert.match(
    workflow,
    /verify_remote_assets true\s+seal_verified_asset_identity\s+require_pinned_release\s+test "\$release_is_draft" = "true"[\s\S]*require_verified_asset_identity\s+if ! gh api \\\n\s+--hostname github\.com \\\n\s+--method PATCH \\\n\s+"repos\/\$GITHUB_REPOSITORY\/releases\/\$release_id" \\\n\s+-F draft=false/,
  );
  assert.match(
    workflow,
    /derive_remote_asset_identity\(\)[\s\S]*console\.log\(`\$\{asset\.id\}\\t\$\{asset\.name\}`\);[\s\S]*seal_verified_asset_identity\(\)[\s\S]*cp "\$remote_assets" "\$verified_asset_identity"[\s\S]*require_verified_asset_identity\(\)[\s\S]*cmp -s "\$verified_asset_identity" "\$remote_assets"/,
  );
  assert.match(
    workflow,
    /contain_published_release\(\)[\s\S]*containment_attempt=1[\s\S]*"\$containment_attempt" -le 3[\s\S]*--method PATCH \\\n\s+"repos\/\$GITHUB_REPOSITORY\/releases\/\$release_id" \\\n\s+-F draft=true[\s\S]*fetch_pinned_release[\s\S]*validate_pinned_release_identity[\s\S]*test "\$release_is_draft" = "true"[\s\S]*Publication state is unknown for pinned release id/,
  );
  assert.match(
    workflow,
    /Post-publication authority failed; enforcing draft containment[\s\S]*contain_published_release[\s\S]*returned to draft containment/,
  );
  assert.match(
    workflow,
    /if \[ "\$release_is_draft" = "false" \]; then[\s\S]*verify_remote_assets true[\s\S]*seal_verified_asset_identity[\s\S]*require_pinned_release[\s\S]*require_verified_asset_identity[\s\S]*Published release authority failed; enforcing draft containment[\s\S]*contain_published_release/,
  );
  assert.doesNotMatch(
    workflow,
    /\(\s*contain_published_release\s*\)\s*\|\|\s*true/,
  );
  ...
  assert.equal(
    packageMetadata.scripts.check,
    "bun run typecheck && bun test --parallel=1 --timeout 20000",
  );
  for (const jobName of ["linux", "macos"] as const) {
    const smoke = parsedWorkflow.jobs[jobName]?.steps?.find(
      (step) => step.name === "Execute inspect and smoke check",
    )?.run;
  ...
  const expectedWindowsRegressionCommand =
    "bun test --parallel=1 --timeout 120000 tests/app/runtime.test.ts tests/core/status-auth.test.ts tests/install/apply.test.ts tests/install/atomic-config.test.ts tests/install/clients.test.ts tests/install/config-formats.test.ts tests/install/configurer-flow.test.ts tests/install/status.test.ts tests/install/writers.test.ts tests/runtime/build-info.test.ts tests/runtime/secure-session.test.ts tests/runtime/secure-store.test.ts tests/runtime/windows-acl.test.ts tests/sana/client.test.ts tests/sana/auth-request.test.ts tests/sana/auth.test.ts tests/sana/auth-e2e.test.ts tests/sana/session-publication.test.ts tests/sync/control.test.ts tests/sync/daemon.test.ts tests/sync/lifecycle.test.ts tests/tools/dispatch-auth.test.ts tests/install/config-transaction.test.ts tests/install/config-transaction-flow.test.ts";
  assert.equal(parsedWorkflow.jobs.windows?.["runs-on"], "windows-2022");
  const windowsSteps = parsedWorkflow.jobs.windows?.steps;
  const windowsBun = windowsSteps?.find((step) =>
    step.uses?.startsWith("oven-sh/setup-bun@")
  );
  assert.deepEqual(windowsBun, {
    uses: "oven-sh/setup-bun@0c5077e51419868618aeaa5fe8019c62421857d6",
    with: {
      "bun-version": "1.3.14",
    },
  });
  const syntaxStep = windowsSteps?.find(
    (step) => step.name === "Validate the Windows installer syntax",
  );
  assert.ok(syntaxStep);
  assert.equal(syntaxStep.shell, "powershell");
  assert.match(
    syntaxStep.run ?? "",
    /System\.Management\.Automation\.Language\.Parser\]::ParseFile/u,
  );
  assert.equal(
    windowsSteps?.find(
  ...
  assert.equal(
    windowsSteps?.find(
      (step) => step.name === "Exercise exact-path Windows runtime replacement",
    )?.run,
    'bun test --parallel=1 --timeout 30000 tests/install/installers.test.ts --test-name-pattern "PowerShell retires an active exact-path runtime"',
  );
  const predecessorStep = windowsSteps?.find(
    (step) => step.id === "predecessor",
  );
  assert.ok(predecessorStep);
  assert.equal(
    predecessorStep.name,
    "Fetch the authenticated v0.4.5 Windows fixture",
  );
  assert.equal(predecessorStep.shell, "powershell");
  assert.deepEqual(predecessorStep.env, {
    GH_TOKEN: "${{ github.token }}",
    PREDECESSOR_TAG: "v0.4.5",
  });
  assert.match(
    predecessorStep.run ?? "",
    /\$env:GITHUB_REPOSITORY[\s\S]*& gh @DownloadArguments/,
  );
  assert.match(
    predecessorStep.run ?? "",
    /"manifest\.json",\s+"manifest\.json\.sha256",\s+"sana-mcp-windows-x64\.exe",\s+"sana-mcp-windows-x64\.exe\.sha256"/,
  );
  assert.match(
    predecessorStep.run ?? "",
    /\$Observed\.Count -ne \$ExpectedAssets\.Count[\s\S]*\$ExpectedAssets -cnotcontains \$Item\.Name/,
  );
  assert.match(
    predecessorStep.run ?? "",
    /"directory=\$Fixture" \| Out-File -LiteralPath \$env:GITHUB_OUTPUT/,
  );
  assert.deepEqual(
    windowsSteps
      ?.filter(
        (step) =>
          step.env &&
          "GH_TOKEN" in step.env &&
          step.env.GH_TOKEN !== "",
      )
      .map((step) => step.id),
    ["predecessor"],
  );
  const buildStep = windowsSteps?.find(
    (step) => step.name === "Build through canonical release configuration",
  );
  assert.ok(buildStep);
  assert.equal(
    buildStep.run,
    "bun scripts/release.ts build --target bun-windows-x64 --outfile sana-mcp-windows-x64.exe",
  );
  const smokeStep = windowsSteps?.find(
    (step) => step.name === "Run tokenless native PowerShell smoke and attest",
  );
  assert.ok(smokeStep);
  ...
  assert.match(
    smokeStep.run ?? "",
    /\[IO\.File\]::Open\([\s\S]*\[IO\.FileShare\]::Read[\s\S]*\$ExecutedSha256[\s\S]*& \$Binary --version[\s\S]*\$ObservedVersion -cne \$PackageVersion/u,
  );
  assert.match(
    smokeStep.run ?? "",
    /\$InspectNames[\s\S]*Compare-Object \$ExpectedNames \$InspectNames[\s\S]*\$Inspect\.mode -cne "standalone"[\s\S]*\$Inspect\.target -cne "bun-windows-x64"/u,
  );
  assert.match(
    smokeStep.run ?? "",
    /\[IO\.File\]::WriteAllText\([\s\S]*\[Text\.UTF8Encoding\]::new\(\$false\)/u,
  );
  assert.doesNotMatch(smokeStep.run ?? "", /Set-Content/u);
  assert.match(smokeStep.run ?? "", /& \$Binary --help/u);
  assert.match(
    smokeStep.run ?? "",
    /\$BinaryLease\.Position = 0[\s\S]*\$PostExecutionSha256[\s\S]*\$PostExecutionSha256 -cne \$ExecutedSha256[\s\S]*"attest"[\s\S]*"--commit-sha", \$env:SOURCE_SHA[\s\S]*"--executed-sha256", \$ExecutedSha256[\s\S]*\$BinaryLease\.Dispose\(\)[\s\S]*\$Attestation\.sourceCommit -cne \$env:SOURCE_SHA[\s\S]*\$Attestation\.sha256 -cne \$ExecutedSha256[\s\S]*\$Attestation\.inspect\.target -cne "bun-windows-x64"[\s\S]*\$Attestation\.inspect\.version -cne \$PackageVersion/u,
  );
  assert.doesNotMatch(
    smokeStep.run ?? "",
    /\$Attestation\.target\b/u,
  );
  const replacementStep = windowsSteps?.find(
  ...
  assert.deepEqual(replacementStep.env, {
    SANA_MCP_TEST_CURRENT_WINDOWS_BINARY:
      "${{ github.workspace }}\\sana-mcp-windows-x64.exe",
    SANA_MCP_TEST_SOURCE_COMMIT: "${{ needs.authorize.outputs.sha }}",
    SANA_MCP_TEST_PREDECESSOR_WINDOWS_FIXTURE_DIR:
      "${{ steps.predecessor.outputs.directory }}",
  });
  assert.ok(!("GH_TOKEN" in replacementStep.env));
  const cleanupStep = windowsSteps?.find(
    (step) => step.name === "Remove the authenticated predecessor fixture",
  );
  assert.ok(cleanupStep);
  assert.equal(
    cleanupStep.if,
    "${{ always() && steps.predecessor.outputs.directory != '' }}",
  );
  assert.equal(cleanupStep.shell, "powershell");
  assert.deepEqual(cleanupStep.env, {
    PREDECESSOR_FIXTURE: "${{ steps.predecessor.outputs.directory }}",
  });
  assert.match(
    cleanupStep.run ?? "",
    /\$Expected = \[IO\.Path\]::GetFullPath[\s\S]*\[string\]::Equals\([\s\S]*\[StringComparison\]::OrdinalIgnoreCase/,
  );
  assert.match(
    cleanupStep.run ?? "",
    /Remove-Item -LiteralPath \$Fixture -Recurse -Force[\s\S]*if \(Test-Path -LiteralPath \$Fixture\)/,
  );
  const predecessorIndex = windowsSteps?.indexOf(predecessorStep) ?? -1;
  const replacementIndex = windowsSteps?.indexOf(replacementStep) ?? -1;
  const cleanupIndex = windowsSteps?.indexOf(cleanupStep) ?? -1;
  const syntaxIndex = windowsSteps?.indexOf(syntaxStep) ?? -1;
  const regressionIndex =
    windowsSteps?.findIndex(
      (step) => step.name === "Run native Windows regression suite",
    ) ?? -1;
  const exactPathIndex =
    windowsSteps?.findIndex(
      (step) =>
        step.name === "Exercise exact-path Windows runtime replacement",
    ) ?? -1;
  const buildIndex = windowsSteps?.indexOf(buildStep) ?? -1;
  const smokeIndex = windowsSteps?.indexOf(smokeStep) ?? -1;
  const uploadIndex =
    windowsSteps?.findIndex((step) =>
      step.uses?.startsWith("actions/upload-artifact@")
    ) ?? -1;
  assert.ok(syntaxIndex >= 0);
  assert.ok(regressionIndex > syntaxIndex);
  assert.ok(exactPathIndex > regressionIndex);
  assert.ok(buildIndex > exactPathIndex);
  assert.equal(smokeIndex, buildIndex + 1);
  assert.equal(predecessorIndex, smokeIndex + 1);
  assert.ok(predecessorIndex >= 0);
  assert.ok(replacementIndex > predecessorIndex);
  assert.ok(cleanupIndex > replacementIndex);
